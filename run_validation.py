...
def expected_output_path(subbatch: Path, benchmark: str, dissolve_units: bool, output_dir: Path) -> Path:
...
def main():
    ...
    skip_if_exists = bool(config.get("validation", {}).get("skip_if_output_exists", False))
    ...
    for i, (dissolve_units, benchmark, subbatch) in enumerate(combination, start=1):
        try:
            if skip_if_exists:
                out_path = expected_output_path(Path(subbatch), benchmark, dissolve_units, output_dir)
                if out_path.exists():
                    logging.info(f"Skipping {subbatch} (benchmark={benchmark}, dissolve={dissolve_units}) because output exists: {out_path}")
                    continue
