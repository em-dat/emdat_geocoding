...
# %%
# 1b. Load GDIS location
logging.info(f"Loading GDIS batch file {GDIS_BATCH_FILE}")
gdis_gdf = load_geocoded_batch(GDIS_BATCH_FILE)
logging.info(f"{len(gdis_gdf)} records loaded")
logging.info(f"Columns: {', '.join(gdis_gdf.columns)}")


# %%
# 2. Dissolve units (optional)
if DISSOLVE_UNITS:
    logging.info("Dissolving llm units")
    ...
    logging.info(f"{len(geocoded_gdf)} geocoded records after dissolving")

    logging.info("Dissolving gdis units")
    gdis_gdf = gdis_gdf.dissolve(
        by="DisNo.",
        aggfunc={
            "geolocation": list,
            "admin_level": list,
    ...
    gdis_gdf.reset_index(inplace=True)
    logging.info(f"{len(gdis_gdf)} gdis records after dissolving")
...
gdis_gdf = gdis_gdf[gdis_gdf["DisNo."].isin(DISNO_OFFICIAL)]
logging.info(f"{len(gdis_gdf)} gdis records filtered based on Dis No.")
...
# %%
# 5a. Run validation (LLM agains EM-DAT)
logging.info(f"Starting EM-DAT validation of {len(geocoded_gdf)} records")
...
# %%
# 5b. Run validation (LLM agains GDIS)
logging.info(f"Starting GDIS validation of {len(geocoded_gdf)} records")
...
for ix, row in geocoded_gdf.iterrows():
    disno = row["DisNo."]
    geom_a = row["geometry"]
    geom_b = gdis_gdf[gdis_gdf["DisNo."] == disno]["geometry"].iloc[0]
    indices: dict[str, float] = calculate_area_indices(geom_a, geom_b)
    results = [
        row["DisNo."],
        row["name"],
    ...
    result_df.loc[ix] = results
logging.info(f"Saving results to {OUTPUT_FILENAME_GDIS}")
result_df.to_csv(OUTPUT_FILENAME_GDIS, index=False)
logging.info(f"Validation complete".upper())
